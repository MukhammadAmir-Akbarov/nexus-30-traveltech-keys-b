...
export default function PlanPage() {
  ...
  // label — номер ВНУТРИ дня, тот же, что стоит в таймлайне ниже: иначе объект
  // оказывается «4» на карте и «1» в плане дня, и это два разных маршрута
  // для того, кто сверяет их глазами.
  const routePoints: RoutePoint[] = days.flatMap((day) =>
    placesOf(day).map(({ place }, index) => ({ place, day: day.day, label: index + 1 })),
  );
